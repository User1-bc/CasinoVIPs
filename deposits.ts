...
export function startDepositChecker(bot: Bot) {
  ...
  setInterval(async () => {
    try {
      ...
      if (!wallet || wallet.includes("TU_DIRECCION")) return;
      ...
      for (const tx of transactions) {
        ...
        try {
          ...
          await query(
            "INSERT INTO transactions (tx_hash, telegram_id, amount, bonus_amount) VALUES ($1, $2, $3, $4)",
            [txHash, telegramId, rawAmount, bonus]
          );

          // Acreditar el balance real y el bono al usuario correspondiente
          await query(
            "UPDATE users SET balance = balance + $1, bonus_balance = bonus_balance + $2 WHERE telegram_id = $3",
            [rawAmount, bonus, telegramId]
          );

          // Marcar la orden pendiente como completada
          await query(
            "UPDATE pending_deposits SET status = 'completed' WHERE id = $1",
            [depositOrderId]
          );
          ...
          try {
            await bot.api.sendMessage(
              telegramId,
              `🎉 **¡DEPÓSITO ACREDITADO CON ÉXITO!** 🎉\n\n` +
              `💵 **Monto Recibido:** $${rawAmount.toFixed(2)} USDT\n` +
              `🎁 **Bono de Casino:** $${bonus.toFixed(2)} USDT\n\n` +
              `¡Tus fondos ya están disponibles para jugar en el casino! 🎰`,
              { parse_mode: "Markdown" }
            );
          } catch (notifErr) {
